...
const VapiWidget: React.FC<VapiWidgetProps> = ({ apiKey, assistantId, config = {} }) => {
    ...
    vapiInstance.on('message', (message: any) => {
      if (message.type === 'transcript') {
        setTranscript((prev) => {
          ...
          console.log('[Vapi transcript updated]', { at: new Date().toISOString(), length: combined.length, preview });
          return next;
        });
      }
    });
  ...
  const startCall = () => {
    if (vapi) {
      vapi.start(assistantId, config as any);
    }
  };
        ...
        >
          ...
          >
            ...
            >
              ...
              ></div>
              <span style={{ fontWeight: 'bold', color: '#333' }}>{isSpeaking ? 'Assistant Speaking...' : 'Listening...'}</span>
            </div>
            <button
              onClick={endCall}
              style={{
          ...
          >
            {transcript.length === 0 ? (
              <p style={{ color: '#666', fontSize: '14px', margin: 0 }}>Conversation will appear here...</p>
            ) : (
